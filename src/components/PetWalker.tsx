...
type PetStyle = CSSProperties & {
  "--tisee-pet-edge-travel": string;
  "--tisee-pet-return-18": string;
  "--tisee-pet-return-44": string;
  "--tisee-pet-return-72": string;
};
...
function PandaFrame({ className, src }: PandaFrameProps) {
  return (
    <Image
      ...
      className={`tisee-pet-frame ${className}`}
      loading="eager"
      draggable={false}
    />
  );
...
export default function PetWalker() {
  ...
  const getTravelForStep = useCallback(
    (targetStep: number) => (targetStep / MAX_ROLL_STEPS) * getMaxTravel(),
    [getMaxTravel],
  );

  const petStyle: PetStyle = {
    transform: `translate3d(${travelX}px, 0, 0)`,
    "--tisee-pet-edge-travel": `${maxTravel}px`,
    "--tisee-pet-return-18": `${maxTravel * 0.8}px`,
    "--tisee-pet-return-44": `${maxTravel * 0.57}px`,
    "--tisee-pet-return-72": `${maxTravel * 0.24}px`,
  };
    ...
    >
      ...
      <span className="tisee-pet-art" aria-hidden="true">
        <span className="tisee-pet-idle-frames">
          <PandaFrame src="/images/panda/panda-idle-1.png" className="tisee-pet-frame-idle-1" />
          <PandaFrame src="/images/panda/panda-idle-2.png" className="tisee-pet-frame-idle-2" />
        </span>
        <span className="tisee-pet-roll-frames" key={rollToken}>
          <PandaFrame
            src="/images/panda/panda-roll-crouch.png"
            className="tisee-pet-roll-frame tisee-pet-roll-frame-crouch"
          />
          <PandaFrame
            src="/images/panda/panda-roll-mid.png"
            className="tisee-pet-roll-frame tisee-pet-roll-frame-mid"
          />
          <PandaFrame
            src="/images/panda/panda-roll-land.png"
            className="tisee-pet-roll-frame tisee-pet-roll-frame-land"
          />
        </span>
      </span>
    </button>
